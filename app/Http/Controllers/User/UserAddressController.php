...
class UserAddressController extends Controller
{
    ...
    {
        $data['title'] = __('User Address');
        $data['addresses'] = Auth::user()->addresses;

        return view('frontend.user_access.profile.personal_info.address.index', $data);
    }
    ...
    public function create()
    {
        ...
        return view('frontend.user_access.profile.personal_info.address.create', $data);
    }

    public function store(AddressRequest $request)
    {

        $checkAddressStatus = $this->address->getFirstByConditions(['ownerable_id' => Auth::user()->id,'is_default' => ACTIVE_STATUS_ACTIVE]);
        ...
        $parameters['ownerable_type'] = get_class(new User());
        $parameters['ownerable_id'] = Auth::user()->id;
        $parameters['is_verified'] = VERIFICATION_STATUS_UNVERIFIED;
        $parameters['is_default'] = $activeStatus;

        if ($this->address->create($parameters))
        {
            return redirect()->route('user-address.index')->with(SERVICE_RESPONSE_SUCCESS, __('Address has been created successfully'));
        }
    ...
    public function edit($id)
    {
        ...
        return view('frontend.user_access.profile.personal_info.address.edit', $data);
    }
    ...
    public function update(AddressRequest $request, $id)
    {
        ...
        {
            return redirect()->route('user-address.index')->with(SERVICE_RESPONSE_SUCCESS, __('Address has been updated successfully'));
        }
    ...
    public function makeAddressDefault(Request $request, $id)
    {
        ...
        $verifiedAddress = $this->address->getFirstByConditions(['ownerable_id' => Auth::user()->id,'is_default' => ACTIVE_STATUS_ACTIVE ]);
        ...
        if (!empty($address) && !empty($verifiedAddress))
        {
            ...
            {
                return redirect()->route('user-address.index')->with(SERVICE_RESPONSE_SUCCESS, __('Successful'));
            }
